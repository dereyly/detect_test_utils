...
def replace_category(src_string):
    def replace_string(src, condition, out):
        dst = out if src == condition else src
        return dst
...
def divide_img(src_img, object_markup_data):
...
for csv_row in csv_rows:
    ...
    if lot_group_name != aug_cfg.lot_group_name_background:
        ...
        if 'dairy' in aug_cfg.csv_path:
            ...
            klass_package_type_code = replace_category(klass_package_type_code)
...
sku_space_package_type_list = sorted(sku_space_package_type_dict.items(), key=operator.itemgetter(1))
for key, value in sku_space_package_type_list:
    f_sku_space_package_type.write(key + ' ' + str(value) + '\n')
...
def augment_and_save_img_and_save_xml(imgs_markup_list_item):
    def save_img_and_xml(dst_img, trans_tl_and_br_coors, objects_markup_data_src, dst_img_and_xml_info):
        ...
        for idx, object_markup_rec in enumerate(objects_markup_data_src):
            ...
            embedding_status = embedding_status if embedding_status is not None else 'none'
